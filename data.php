...
echo "<thead><tr><th>date/heure(IN)</th><th>Lot ID</th><th>Alias</th><th>RTP</th><th>Emplacement</th><th>Résultat</th><th>name</th><th>date\heure (OUT)</th></tr></thead>";
echo "<tbody>";
...
foreach ($resultats_array as $ligne) {
    ...
    echo "<tr class='" . ($ligne['confirmation'] == 'ok' ? 'ok' : 'ko') . "'>";
    echo "<td>".$ligne['date_heur']."</td><td>".$ligne['lot_id']."</td><td>".$ligne['alias']."</td><td>".$ligne['rtp']."</td><td>".$ligne['emplacement']."</td><td>".$ligne['confirmation']."</td><td>".$ligne['username']."</td><td>".$ligne['date_heur1']."</td>";
    echo "</tr>";
}
